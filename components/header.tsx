...
const Header = ({ headerClasses }: any) => {
  ...
  return (
    <div className={` ${headerClasses}    bg-[#FFFFFF] border border-t-0 border-l-0 border-r-0  border-b-black w-full`} >
      <div className="mobile_center md:py-5 pt-5  flex-between ">
        <section className=" flex-center space-x-7">
          ...
          {status === 'authenticated' &&
            <section onClick={() => setShowSearch(true)} className=" md:flex hidden py-[9px] px-3 bg-gray-100 rounded-full  items-center gap-2">
      ...
      <section className="mobile_center my-4  md:hidden flex">
        {status === 'authenticated' &&
          <section onClick={() => setShowSearch(true)} className=" flex w-full  py-[9px] px-3 bg-gray-100 rounded-full  items-center gap-2">
            <span><CiSearch size={20} /></span>
            <input
              className="sohne border-0 text-sm font-bold focus:outline-none focus:ring-0  bg-gray-100"
              placeholder="Search"

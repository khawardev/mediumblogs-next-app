...
const page = async ({ params }: { params: { storyId: string } }) => {
    ...
    const publishedStory: any = await getStorybyId(params?.storyId, true, 'published');
    ...
    console.log(publishedStory, '--------- publishedStory Stories -----------');

    return (
        <PublishedStory clapByUser={ClapsCountByUser?.clapCount} favStatus={favStatus} noOfComments={noOfComments} allClaps={allClaps} currentUser={currentUser} publishedStory={publishedStory} userParams={publishedStory?.auther?.id} />
    )
}
